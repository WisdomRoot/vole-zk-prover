...
pub fn repeat_iowe(
    block_size: usize,
    q: usize,
    binomial_coeffs: &Vec<Vec<BigUint>>,
) -> DecimalMatrix {
    let l = block_size + 1;
...
/// Batch n-choose-k, exploting patterns in the binomial coefficient for efficient batch calculation in a particular range
/// This only does it for square nxn matrices but coudlbe easily modified to support arbitrary matrix dimensions
...
pub fn calc_iowe_entry(
    input_hamming: usize,
    output_hamming: usize,
    block_size: usize,
    binomial_coeffs: &Vec<Vec<BigUint>>,
) -> BigUint {
    if input_hamming == 0 {
        return if output_hamming == 0 {
...
pub fn calc_transition_prob(
    ...
    BigDecimal::from_str(&iowe.to_string()).unwrap()
        / BigDecimal::from_str(&denominator.to_string()).unwrap()
}

/// Calculates column of IOWE matrix for the accumulate ode
pub fn calc_iowe_column(
    output_hamming: usize,
    block_size: usize,
    binomial_coeffs: &Vec<Vec<BigUint>>,
) -> Vec<BigUint> {
    (0..block_size + 1)
        .map(|ih| calc_iowe_entry(ih, output_hamming, block_size, binomial_coeffs))
        .collect_vec()
}

pub fn calc_transition_prob_column(
    output_hamming: usize,
    block_size: usize,
    binomial_coeffs: &Vec<Vec<BigUint>>,
) -> DecimalVec {
    let v = (0..block_size + 1)
        .map(|ih| calc_transition_prob(ih, output_hamming, block_size, binomial_coeffs))
        .collect_vec();
    DecimalVec(v)
}
...
pub fn calc_iowe_matrix_cols(block_size: usize) -> Vec<Vec<BigUint>> {
    ...
    (0..block_size + 1)
        .map(|ih| calc_iowe_column(ih, block_size, bcm))
        .collect_vec()
}

pub fn calc_transition_prob_matrix_cols(block_size: usize) -> DecimalMatrix {
    let bcm = &n_choose_k_square_matrix(block_size);
    let m = (0..block_size + 1)
        .map(|ih| calc_transition_prob_column(ih, block_size, bcm))
        .collect_vec();
    DecimalMatrix(m)
}
...
pub fn values_for_rma_code(
    q: usize,
    block_size: usize,
    num_accumulators: usize,
) -> (usize, DecimalMatrix, DecimalMatrix) {
...
pub fn expected_num_outputs_with_weight(
    k: usize,
    outer_iowe: &DecimalMatrix,
    inner_transition_prob: &DecimalMatrix,
    h: usize,
) -> BigDecimal {
    assert!(
        h < outer_iowe.0.len(),
        "it is not sensible to call for an output weight < the block size."
    );
...
pub fn main() {
    ...
    println!(
        "prob of minimum distance under all numbers precenting {} are at most {}",
        d,
        DecimalVec(for_all_til_d)
    );
}
...
impl DecimalVec {
    ...
    pub fn from_f64_vec(v: Vec<f64>) -> Self {
        Self(
            v.iter()
                .map(|x| BigDecimal::from_f64(*x).unwrap())
                .collect_vec(),
        )
    }
    pub fn is_close_to(&self, rhs: &Self, tol: f64) -> bool {
        self.0
            .iter()
            .zip(rhs.0.iter())
            .all(|(a, b)| (a - b).abs() < BigDecimal::from_f64(tol).unwrap())
    }
}
impl std::fmt::Display for DecimalVec {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        self.0.iter().for_each(|d| {
            string.push_str(&d.round(8).to_string());
            string.push_str(", ")
        });
        write!(f, "{}", string)
    }
...
impl DecimalMatrix {
    pub fn mul(&self, rhs: &Self) -> Self {
        ...
        for row in self.0.iter() {
            output_rows.push(DecimalVec(
                transposed.0.iter().map(|col| col.dot(row)).collect_vec(),
            ));
        }
        Self(output_rows)
    }
...
mod test {
    ...
    fn iowe_matrix() {
        ...
        assert_eq!(
            ...
            vec![
                vec![1, 0, 0, 0]
                    .iter()
                    .map(|x| BigUint::from_u8(*x).unwrap())
                    .collect_vec(),
                vec![0, 1, 2, 0]
                    .iter()
                    .map(|x| BigUint::from_u8(*x).unwrap())
    ...
    fn tprob_matrix() {
        ...
        assert!(m.is_close_to(&and, 1e-10));
    }
    ...
    fn test_batch_n_choose_k() {
        ...
        for (i, row) in m.iter().enumerate() {
            for (j, item) in row.iter().enumerate() {
                assert_eq!(item.to_usize().unwrap(), binomial(i, j));
            }
        }
    }
    ...
    fn multiple_acc() {
        ...
        assert!(once.is_close_to(
            &DecimalMatrix(vec![
                DecimalVec::from_f64_vec(vec![1.0, 0.0, 0.0, 0.0]),
                DecimalVec::from_f64_vec(vec![
                    0.0,
                    0.3333333333333333,
                    0.3333333333333333,
                    0.3333333333333333
